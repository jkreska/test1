...
$paged_comments->use_wp_config = true;
...
// override default values with WP paging config and custom post values
function paged_comments_update_values($post_id=null)
{
	global $paged_comments;
	// comments per page
	if ($paged_comments->use_wp_config && (int)get_option('comments_per_page') > 0) {
		$paged_comments->per_page = (int)get_option('comments_per_page');
	}
	$val = paged_comments_get_custom('comments_per_page', $post_id);
	if (!empty($val)) $paged_comments->per_page = (int)$val;
	// comment ordering
	if ($paged_comments->use_wp_config && get_option('comment_order')) {
		$paged_comments->ordering = strtoupper(get_option('comment_order'));
	}
	$val = strtoupper(paged_comments_get_custom('comment_ordering', $post_id));
	if (($val == 'ASC') || ($val == 'DESC')) $paged_comments->ordering = $val;
	// default page
	if ($paged_comments->use_wp_config && get_option('default_comments_page')) {
		if (get_option('default_comments_page') == 'newest') {
			$paged_comments->default_page = 'last';
		} else {
			$paged_comments->default_page = 'first';
		}
	}
}
