...
function email_js() {
	...
	echo '/* <![CDATA[ */'."\n";
	echo "\t".'var email_ajax_url = \''.WP_PLUGIN_URL.'/wp-email/wp-email.php'."';\n";
	...
	echo "\t".'var email_text_max_friend_names_allowed = \''.js_escape(sprintf(__('- Maximum %s Friend Name(s) allowed', 'wp-email'), $email_max)).'\';'."\n";
	...
	echo "\t".'var email_text_max_friend_emails_allowed = \''.js_escape(sprintf(__('- Maximum %s Friend Email(s) allowed', 'wp-email'), $email_max)).'\';'."\n";
	...
	if(@file_exists(TEMPLATEPATH.'/email-css.css')) {
		...
		echo '<link rel="stylesheet" href="'.WP_PLUGIN_URL.'/wp-email/email-css.css" type="text/css" media="screen" />'."\n";
	}	
	echo '<!-- End Of Script Generated By WP-EMail 2.31 -->'."\n";
}
...
function email_form($popup = false, $echo = true, $subtitle = true, $div = true, $error_field = '') {
	...
	if (not_spamming()) {
		if(not_password_protected()) {
			...
			if($email_image_verify) {
				...
				$output .= '<img src="'.WP_PLUGIN_URL.'/wp-email/email-image-verify.php" width="55" height="15" alt="'.__('E-Mail Image Verification', 'wp-email').'" /><input type="text" size="5" maxlength="5" id="imageverify" name="imageverify" class="TextField" />'."\n";
				$output .= '</p>'."\n";
			}
	...
	$output .= '<div id="wp-email-loading" class="wp-email-loading"><img src="'.WP_PLUGIN_URL.'/wp-email/images/loading.gif" width="16" height="16" alt="'.__('Loading', 'wp-email').' ..." title="'.__('Loading', 'wp-email').' ..." class="wp-email-image" />&nbsp;'.__('Loading', 'wp-email').' ...</div>'."\n";
	if($div) {
		$output .= '</div>'."\n";
	}
	email_removefilters();
